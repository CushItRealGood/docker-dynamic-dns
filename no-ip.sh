...
for ((COUNTER = 1; COUNTER <= DOMAINS; COUNTER++)); do
	if [[ -z $(eval echo \$$(echo "USER$COUNTER")) ]]; then
...
for ((COUNTER = 1; COUNTER <= DOMAINS; COUNTER++)); do
	if [[ -z $(eval echo \$$(echo "PASSWORD$COUNTER")) ]]; then
...
for ((COUNTER = 1; COUNTER <= DOMAINS; COUNTER++)); do
	if [[ -z $(eval echo \$$(echo "HOSTNAME$COUNTER")) ]]; then
...
if [ -z "$HOSTNAME" ] && [ -z "$HOSTNAME1" ]; then
...
if [ -z "$DOMAINS" ]; then
	...
	while :; do
		for ((COUNTER = 1; COUNTER <= DOMAINS; COUNTER++)); do
			if [[ -z $(eval echo \$$(echo "USER$COUNTER")) && -z $(eval echo \$$(echo "PASSWORD$COUNTER")) ]]; then
				...
				echo " "
				eval echo is the status for \$$(echo "HOSTNAME$COUNTER") # The additional echo commands provide visual indicator log witout passwords
